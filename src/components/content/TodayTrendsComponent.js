...
for (let x = 1; x <= 31; x++) {
    data.push({ x: x, y: 10 }) //Math.floor(Math.random() * (100)) })
}
        ...
        return (
            <Row flexGrow={1} className={css(styles.container)}
                ...
                <Column wrap flexGrow={7} flexBasis="735px" className={css(styles.graphSection)}
                    ...
                    <Row wrap horizontal="space-between">
                        <Column>
                            ...
                            <span className={css(styles.graphSubtitle)}>as of 25 March 2020, 09:41 PM</span>
                        </Column>
                ...
                <Column flexGrow={3} flexBasis="342px" breakpoints={{ 1024: css(styles.stats) }}>
                    {this.renderStat('Resolved', '0')}
                    {this.renderStat('Received', '0')}
                    {this.renderStat('Average first response time', '0m')}
                    {this.renderStat('Average response time', '0h 0m')}
                    {this.renderStat('Resolution within SLA', '0%')}
                </Column>
            </Row>
        );
    }
